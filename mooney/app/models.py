...
class Account(db.Model):
    ...
    start_balance = db.Column(db.Float(32), default=0.00)
...
class Category(db.Model):
    ...
    created_at    = db.Column(db.DateTime, index=True, default=datetime.utcnow)
    last_modified = db.Column(db.DateTime, index=True, default=datetime.utcnow)
    budget = db.Column(db.Float(32), default=0.00)
    user_id       = db.Column(db.Integer, db.ForeignKey('user.id'))
...
class Transaction(db.Model):
    ...
    amount = db.Column(db.Float(32), default=0.00, index=True)
...
class Assets(db.Model):
    ...
    previous_close = db.Column(db.Float(32))
    historic_price = db.Column(db.String(), nullable=True)
    quantity       = db.Column(db.Integer)
    cost           = db.Column(db.Float(32))
    purchase_date  = db.Column(db.Date)
    purchase_price = db.Column(db.Float(32))
